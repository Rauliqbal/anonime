      ...
      <>
         ...
         >
            {Array.from(sliders)
               .slice(0, 8)
               .map((slider, index) => (
                  <SwiperSlide className="h-44 sm:h-72 md:h-96 lg:h-[30rem] rounded-2xl overflow-hidden" key={index}>
                     <Navlink to={`/detail/${slider.mal_id}`} className=" relative rounded-xl object-cover w-full group-hover:scale-105 transition-all duration-400 ease-out">
                        <img className="absolute w-full h-full object-cover" src={slider.trailer.images.maximum_image_url} alt="" />
